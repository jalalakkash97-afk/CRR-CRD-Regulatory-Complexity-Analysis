'''Version: 09.06.2026
Funktion zur Identifizierung einfacher interner Artikelverweise im CRR-Text.

Diese Datei ist analog zu einzelverweise_crd.py aufgebaut.
Sie erkennt einfache Verweise wie "Article 30" oder "Article 4(1)".
...
# Definiert eine Hilfsfunktion, die prueft, ob ein externer Artikelverweis auf die CRD zeigt.
def ist_crd_artikelverweis(CFR_Text, match_start, match_end, paraend):
    ...
    # Gibt False zurueck, wenn kein CRD-Hinweis gefunden wurde.
    return False


# Definiert die eigentliche Funktion zur Suche einfacher CRR-Artikelverweise.
def einzelverweise_crr(ParagraphSign, parabegin, paraend, CFR_Text, ParagraphList, current_paragraph):

    # Erstellt eine leere Liste fuer alle internen Einzelverweise im aktuellen Artikel.
    Einzelverweise = []

    # Erstellt eine leere Liste fuer externe Einzelverweise.
    ExterneEinzelverweise = []

    # Erstellt eine leere Liste fuer externe Einzelverweise, die speziell auf die CRD zeigen.
    CRDEinzelverweise = []
    ...
    # Gibt interne, externe und davon CRD-bezogene Einzelverweise getrennt an main.py zurueck.
    return Einzelverweise, ExterneEinzelverweise, CRDEinzelverweise
